...
def draw_boxes(img_names, boxes_dicts, class_names, model_size):
    ...
    Returns:
        None.
    """
    colors = np.array((np.random.randint(255, size=(255,3)))*255).astype(np.uint8)
    ...
    for cls in range(len(class_names)):
        ...
        if np.size(boxes) != 0:
            for box in boxes:
                ...
                print("HAKDSHKJHAKSJHFKJAHSFKJHADKFJHKJADHF   ",xy[0])
                kotak.append([xy[0],xy[3],xy[1],xy[2]])
                kotak2.append([xy[0],xy[1],xy[2],xy[3],box[4]])
                kelas.append(class_names[cls])
